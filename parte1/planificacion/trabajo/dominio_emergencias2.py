...
def choose_victim(state):
    amb_loc = state.coordinates[state.ambulance["Location"]]
    min_distance = 999999
    victim = None
    for v in state.victims:
        v = state.victims[v]
        v_loc = state.coordinates[v["Location"]]
        loc_distance = distance(v_loc, amb_loc)
...
def unload_ambulance_op(state):
    if state.in_ambulance:
        ...
        state.victims.pop(state.victim["Nombre"])
        return state
    return False
...
def deliver_by_ambulance1(state,goal):
    if not choose_victim(state): return False
    x=state.victim["Location"]
    y=goal.final
    ...
    if x!=y and gt >= gv:
        return [('go_to_victim',goal), ('load_ambulance_op',), ('evaluate_victim',goal),('go_to_hospital',goal), ('deliver_victim',goal)]
    return False
...
state1.victims = {
    "Pedro":{"Nombre":"Pedro","Edad":45,"Location":"Av Cid","Gravedad":9},
    "Alvaro":{"Nombre":"Alvaro","Edad":25,"Location":"Benimaclet","Gravedad":5},
    # "Maria":{"Nombre":"Maria","Edad":55,"Location":"Colon","Gravedad":7},
    # "Ana":{"Nombre":"Ana","Edad":36,"Location":"Ayuntamiento","Gravedad":3},
    # "Jose":{"Nombre":"Jose","Edad":36,"Location":"Ayuntamiento","Gravedad":3},
    # "Laura":{"Nombre":"Laura","Edad":36,"Location":"Ayuntamiento","Gravedad":1},
}

state1.victim = None # default pedro
