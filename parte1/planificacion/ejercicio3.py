...
def travel_by_car0(state,goal):
    x=state.location
    y=goal.final
    if x!=y:
        return [('load_car_op', 'c0'), ('travel_to_city',goal), ('unload_car_op', )]
    return False

def travel_by_car1(state,goal):
    x=state.location
    y=goal.final
    if x!=y:
        return [('load_car_op', 'c1'), ('travel_to_city',goal), ('unload_car_op', )]
    return False

def travel_by_car2(state,goal):
    x=state.location
    y=goal.final
    if x!=y:
        return [('load_car_op', 'c2'), ('travel_to_city',goal), ('unload_car_op', )]
    return False

pyhop.declare_methods('travel',travel_by_car0, travel_by_car1, travel_by_car2)
print('')
pyhop.print_methods()
...
result=pyhop.pyhop(state1,[('travel',goal1)],verbose=1)
